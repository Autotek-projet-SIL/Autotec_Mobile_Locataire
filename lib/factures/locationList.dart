...
//  DateTime dateFacture;
  late int montant;
  // String heure;
  late String pointDepart;
  late String pointArrive;
  late String marque;
  late String modele;

  Future<List<Location>> _getLocations() async {
    var url = Uri.https("autotek-server.herokuapp.com",
        "/gestionlocations/get_locations_termines_by_locataire/${UserCredentials.uid!}");
    // print (Url.toString());
                          ...
                          return GestureDetector(
                              onTap: () {
                                ...
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => FactureDetails(
                                          nomLoc: nom,
                              ...
                              child: WidgetViewTrip(
                                carName: "${data![index].modele}",
                                carPrice: "${data[index].montant} DA",
                                start:
                                    "${data[index].dateDebut.day}-${data[index].dateDebut.month}-${data[index].dateDebut.year} à ${data[index].heureDebut}",
                                end:
                                    "${data[index].dateDebut.day}-${data[index].dateDebut.month}-${data[index].dateDebut.year} à ${data[index].heureFin}",
                                press: () {
                                  numeroChassis =
                                      "${data[index].numeroChassis}";
                                  heureDebut = data[index].heureDebut;
                                  heureFin = data[index].heureFin;
                                  region = "${data[index].region}";
                                  dateDebut = data[index].dateDebut;
